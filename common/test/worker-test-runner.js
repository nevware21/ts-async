...
(function() {
    ...
    function loadAndRunTests(payload) {
        ...
        try {
            // Load Mocha into worker and expose BDD globals
            ...
            // Resolve entrypoints from actual wrapper keys
            ...
            var resolvedEntrypoints = wrapperKeys.filter(function(key) {
                return /\/test\/src\/.*\.test\.(ts|js)$/.test(key);
            });
            ...
            // Execute entrypoints via karma-typescript commonjs runtime
            importScripts(commonjsUrl);

            // Run Mocha and report results back to Karma
    ...
    // Signal to adapter that worker is ready
